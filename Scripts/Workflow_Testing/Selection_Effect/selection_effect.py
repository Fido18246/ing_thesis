...
def model_testing(data_path_full, data_path_clustered, output_path, dir_name):
    ...
    feature_folders = list(map(str, sorted(np.asarray(os.listdir(data_path_full), dtype=int))))
    ...
    for i, n_features in enumerate(feature_folders):
        print(f'In progress {i+1} of {len(feature_folders)} \t {n_features}')

        CSV_names = np.asarray(os.listdir(f'{data_path_full}{n_features}'))
        CSV_names_clustered = np.asarray(os.listdir(f'{data_path_clustered}{n_features}'))

        Xy_full = list()
        Xy_clustered = list()

        for ii, current_csv_name in enumerate(zip(CSV_names, CSV_names_clustered)):

            Xy_full.append(pd.read_csv(f'{data_path_full}{n_features}/{current_csv_name[0]}'))
            Xy_clustered.append(pd.read_csv(f'{data_path_clustered}{n_features}/{current_csv_name[1]}'))

        # LinearSVC
        t0 = timeit.default_timer()
        dict_LinearSVC[n_features] = cross_validation_LinearSVC(Xy_full, Xy_clustered)
        t1 = timeit.default_timer()
        dict_LinearSVC[n_features].append(t1-t0)

        # SVC
        t0 = timeit.default_timer()
        dict_SVC[n_features] = cross_validation_SVC(Xy_full, Xy_clustered)
        t1 = timeit.default_timer()
        dict_SVC[n_features].append(t1-t0)

        # RF
        t0 = timeit.default_timer()
        dict_RF[n_features] = cross_validation_RF(Xy_full, Xy_clustered)
        t1 = timeit.default_timer()
        dict_RF[n_features].append(t1-t0)

    # Saving time & metrics as DataFrame
    df_LinearSVC = pd.DataFrame.from_dict(dict_LinearSVC, orient='index').reset_index()
    df_LinearSVC.columns = ['n_features', 'Accuracy_mean', 'Jaccard_index_mean', 'Dice_coefficient_mean', 'Accuracy_std', 'Jaccard_index_std', 'Dice_coefficient_std', 'Time']
    ...
    df_SVC.columns = ['n_features', 'Accuracy_mean', 'Jaccard_index_mean', 'Dice_coefficient_mean', 'Accuracy_std', 'Jaccard_index_std', 'Dice_coefficient_std', 'Time']
    df_SVC.to_csv(f'{output_path}SVC.csv', index=False)

    df_RF = pd.DataFrame.from_dict(dict_RF, orient='index').reset_index()
    df_RF.columns = ['n_features', 'Accuracy_mean', 'Jaccard_index_mean', 'Dice_coefficient_mean', 'Accuracy_std', 'Jaccard_index_std', 'Dice_coefficient_std', 'Time']
...
def source_model_testing():
    ...
    for i, curr_suffix in enumerate(folders):
        ...
        DATA_PATH_FULL = f'./Results/Selecting_Features/FoI_S_Xy_{curr_suffix}_{n_clusters}/'
        DATA_PATH_CLUSTERED = f'./Results/Selecting_Features/FoI_C_S_Xy_{curr_suffix}_{n_clusters}/'
        OUTPUT_PATH = './Results/Workflow_Testing/'
        DIR_NAME = f'Selection_effect_exploring_{curr_suffix}_C_{n_clusters}'

        model_testing(DATA_PATH_FULL, DATA_PATH_CLUSTERED, OUTPUT_PATH, DIR_NAME)

        t1 = datetime.datetime.now()

        print(f'Selection efect testing: {t1 - t0}')
