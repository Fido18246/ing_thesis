...
def cross_validation_SVC(Xy_full_paths, X_clustered, y_clustered, output_path):
    """
    Perform cross-validation with Support Vector Classifier (SVC).
    ...
    for i, (train_index, test_index) in enumerate(k_fold.split(Xy_full_paths)):
        ...
        # SVC - fit
        svc = SVC(C=0.015625, gamma=0.015625, kernel='rbf')
        ...
        for i, index in enumerate(test_index):
            ...
            # SVC - Predicting
            y_hat = svc.predict(X_test.values)
    ...
    data = {'names': names, 'Dice_coefficient': np.array(score)}
...
def best_model(data_path_full, data_path_clustered, output_path, dir_name):
    """
    Perform cross-validation with Support Vector Classifier (SVC) and save the results.
    ...
    cross_validation_SVC(Xy_full_paths, X_clustered, y_clustered, output_path)
